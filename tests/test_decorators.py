...
@pytest.mark.parametrize(
    "first_num, second_num, expected", [(1, 2, "my_function ok"), (-1, 2, "my_function ok"), (1, -2, "my_function ok")]
)
...
@pytest.mark.parametrize(
    ...
    [
        ("", 2, "my_function error: " 'can only concatenate str (not "int") to str. ' "Inputs: ('', 2), {}"),
        (1, "2", "my_function error: " "unsupported operand type(s) for +: 'int' and 'str'. " "Inputs: (1, '2'), {}"),
        (
            None,
            2,
            "my_function error: " "unsupported operand type(s) for +: 'NoneType' and 'int'. " "Inputs: (None, 2), {}",
        ),
        (
            1,
            None,
        ...
        (
            [1, 2],
            None,
            "my_function error: " 'can only concatenate list (not "NoneType") to list. ' "Inputs: ([1, 2], None), {}",
        ),
        ([1, 2], 3, "my_function error: " 'can only concatenate list (not "int") to list. ' "Inputs: ([1, 2], 3), {}"),
        (
            {"1": "2"},
            None,
            "my_function error: "
